...
class STULayer2D(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # 2D RFFT over spatial dims
        Xf = torch.fft.rfft2(z, dim=(-2, -1))  # (B, d, Nx, Ny_r) complex

        # Apply basis filters in freq
        Phi = self.Phi_f.conj().unsqueeze(0).unsqueeze(0)     # (1, 1, L, Nx, Ny_r)
        Uf  = Xf.unsqueeze(2) * Phi                            # (B, d, L, Nx, Ny_r) complex

        # Keep Theta real: mix real/imag parts separately
        A = Uf.real  # (B, d, L, Nx, Ny_r)
        B = Uf.imag  # (B, d, L, Nx, Ny_r)

        # Mix d -> h and sum over l directly in freq
        # Result: (B, h, Nx, Ny_r)
        Sf_re = torch.einsum("bdlxy,lhd->bhxy", A, self.Theta)
        Sf_im = torch.einsum("bdlxy,lhd->bhxy", B, self.Theta)

        Sf = torch.complex(Sf_re, Sf_im)  # (B, h, Nx, Ny_r) complex

        # One inverse transform total
        S = torch.fft.irfft2(Sf, s=(self.grid_size_x, self.grid_size_y), dim=(-2, -1))  # (B, h, Nx, Ny)
