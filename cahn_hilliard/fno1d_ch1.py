...
x_train, x_test, y_test, train_loader, test_loader = read_single_data(
    ...
    reduced_resolution_t=reduced_resolution_t*8,
    reduced_batch=1,
    test_ratio=0.1)
...
model = FNO1d(in_channels=in_channel, out_channels=out_dim, modes=16, width=64, initial_step=10).to(device)
...
"""
 bsub -q normal -gpu num=1:mode=exclusive_process -M 80G -n 32  -o /baselines/fno/ch1_fno_rt8.txt python -u /cahn_hilliard/fno1d_ch1.py
 

bsub -q normal -gpu num=1:mode=exclusive_process -M 200G -n 32 \
  -o /baselines/fno/ch1_fno.txt \
  python -u /cahn_hilliard/fno1d_ch1.py \
  --random_seed 0
