...
model = WNO1d(width=64, level=16, layers=4, size=64, wavelet='db1', in_channel=in_channel, out_channel=out_dim, grid_range=1).to(device)
...
"""
 bsub -q normal -gpu num=1:mode=exclusive_process -M 80G -n 32  -o /baselines/wno/ch1_wno.txt python -u /cahn_hilliard/wno1d_ch.py
 

bsub -q normal -gpu num=1:mode=exclusive_process -M 200G -n 32 \
  -o /baselines/wno/ch1_wno.txt \
  python -u /cahn_hilliard/wno1d_ch.py \
  --random_seed 0
