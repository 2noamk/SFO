...
def main():
    ...
    x_train, x_test, y_test, train_loader, test_loader = read_single_data(
        ...
        initial_step=args.initial_step, # 1
        reduced_resolution=args.reduced_resolution, #4
        reduced_resolution_t=args.reduced_resolution_t, #1
        reduced_batch=1,
        test_ratio=args.test_ratio, #0.1
    )
...
"""
 bsub -q normal -gpu num=1:mode=exclusive_process -M 200G -n 32  -o /ch_boundary.txt python -u /cahn_hilliard/stu_ch_mlp.py

  

  
bsub -q normal -gpu num=1:mode=exclusive_process -M 200G -n 32 \
  -o /ch_20_64_stu_mlp_4_r.txt \
  python -u /cahn_hilliard/stu_ch_mlp.py\
  --num_eigenfunc 4 \
  --lifting_dim 64 \
  --random_seed 0 
  
bsub -q normal -gpu num=1:mode=exclusive_process -M 200G -n 32 \
  -o /ch_20_64_stu_mlp_8_r.txt \
  python -u /cahn_hilliard/stu_ch_mlp.py\
  --num_eigenfunc 8 \
  --lifting_dim 64 \
  --random_seed 0 
