...
# 2) Core STU 
# =========================================================
class ChannelMixer_lr(nn.Module):
    """
    - Per-mode depthwise scaling theta_l (vector length d)
    - Sum over modes
    - Single shared global low-rank mix P (Q^T x)

    Input:  U  (B, d, L, n)
    Output: Umix  (B, d, n)
    """
    def __init__(self, lifting_dim: int, rank: int):
        super().__init__()
        self.d = lifting_dim
        self.r = rank

        # Shared low-rank mixing:  Q ∈ R^{d×r}, P ∈ R^{d×r}
        self.Q = nn.Parameter(torch.empty(self.d, self.r))
        self.P = nn.Parameter(torch.empty(self.d, self.r))
    ...
    def forward(self, S: torch.Tensor) -> torch.Tensor:
        ...
        # --------------------------------------------------------
        # 3. Shared low-rank pointwise mixing: P (Q^T S)
        # --------------------------------------------------------

        # Q^T S: (B, r, n)
        tmp = torch.einsum("dr,bdn->brn", self.Q, S)

        # P (tmp): (B, d, n)
        Y = torch.einsum("dr,brn->bdn", self.P, tmp)
...
class ChannelMixer(nn.Module):
    ...
    Output: Umix  (B, d, n)
    """
    def __init__(self, lifting_dim: int):
        super().__init__()
        self.d = lifting_dim

        # Shared low-rank mixing:  Q ∈ R^{d×r}, P ∈ R^{d×r}
        self.M = nn.Parameter(torch.empty(self.d, self.d))
    ...
    def forward(self, S: torch.Tensor) -> torch.Tensor:
        ...
        B, d, n = S.shape
        assert d == self.d 
        S_mix = torch.einsum("hd,bdn->bdn", self.M, S)
...
class STULayer(nn.Module):
    ...
    """
    def __init__(self, lifting_dim: int, grid_size: int, Phi_f: torch.Tensor, rank: int = 8):
        super().__init__()
        assert Phi_f.ndim == 2
        ...
        self.theta = nn.Parameter(torch.ones(self.num_hilbert_functions, self.lifting_dim))

        # learnable mixing per mode
        self.M = nn.Parameter(torch.empty(self.lifting_dim, self.lifting_dim))
        nn.init.kaiming_uniform_(self.M, a=math.sqrt(5))
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # Multiply by per-mode scalers
        # theta is (L, d)
        theta_broadcast = self.theta.transpose(0, 1).unsqueeze(0).unsqueeze(-1)
        # shape: (1, d, L, 1)

        # U: (B, d, L, n)
        U_scaled = U * theta_broadcast

        # Sum over modes: S = sum_l U_l_scaled
        S = U_scaled.sum(dim=2) # (B, d, n)
        
        # Per-mode channel mixing
        S_mix = S#self.M(S)  # (B, d, n)
        
        S_mix = torch.einsum("hd,bdn->bdn", self.M, S)
        
        # GLU
        V = self.glu_val(S_mix)
        G = torch.sigmoid(self.glu_gate(S_mix))
        out = V * G
        ...
        *,
        ...
        self.layers = nn.ModuleList([
            STULayer(
                ...
                grid_size=self.grid_size, 
                Phi_f=Phi,
                rank=low_rank_channel_mix) for _ in range(self.depth_space)
        ])
