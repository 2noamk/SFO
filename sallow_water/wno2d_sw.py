...
model = WNO2d(width=64, level=16, layers=4, size=[64,64], wavelet='db1', in_channel=in_channel, out_channel=out_dim, grid_range=[1,1]).to(device)
...
bsub -q normal -gpu num=1:mode=exclusive_process -M 200G -n 32 \
  -o /baselines/wno/sw_wno.txt \
  python -u /sallow_water/wno2d_sw.py \
  --random_seed 0
